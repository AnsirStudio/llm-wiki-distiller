...
1. 扫描 `pending.md` 全文，找出所有 `- [x]` 开头的条目（包括它下面缩进的子说明，整块一起算）。
2. 直接删除这些已打勾的条目，不挪到 `log.md`、不存档、不另写摘要——勾选本身就是用户给的删除授权，不需要再向用户确认一次。
3. 未打勾（`- [ ]`）的条目原样保留，不因为审查模式自己判断"这条不重要"就删除或打勾——那是用户的判断。
