...
候选数据缺口写入 `nextstep.md` 前，先过三条门槛，**三条都不满足就不写**（最多在 summary 正文一句话带过，不算正式缺口）：

1. 缺失是否直接影响这条结论/这个判断的可信度或可执行性（不是“任何细节都没说全”）；
2. 是否涉及用户可能要做决策的领域（定价/合规/医疗/法律/选型）；
3. 用户是否真的可能回头去查（而不是纯粹的“创作者没展开的技术细节”）。

默认排除：口播名称是否完全对应官方拼写、单条统计数字的具体出处机构、未展开的技术架构细节、是否为商业推广声明（这类“未声明”是常态，不必逐条记录）——除非这条数据本身就是结论的论证核心。单条来源的数据缺口数量控制在 0-3 条以内，只挑最关键的，不是逐条罗列这个来源没说什么。
...
- `nextstep.md` 只记录通过上面“数据缺口过滤”门槛的数据缺口（数量克制，不逐条罗列），以及待确认 self 推断、待深挖对象。
- 断链、重复页、未填字段已检查。
