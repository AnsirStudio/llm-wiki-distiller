...
- 创作者/博主类 entity（见 `schemas/wiki.md` Entity 子模板）每次新增一条来源时：只在该页`已蒸馏作品`表格加一行；不要在`时间线`再加一行重复信息；不要因为这条来源提到新工具/概念就追加 `tags`（走 `related` 互链）；不要把这条来源的链接写进 entity 的 `original_url`（只放主页类链接，见 `schemas/common.md`）。
