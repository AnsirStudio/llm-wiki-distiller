...
- YAML `original_url` 只放核心来源，用于快速读取和证据定位；搜索补充、背景资料、延伸阅读放正文末尾的 `## 来源`，不要塞进 frontmatter。不要在 wiki/self YAML 中使用 `sources` 字段。
...
- 不依赖脚本完成基础工作。skill 应能由 agent 直接读写 Markdown 和文件夹完成。
